...
async def generate_report(request: ReportRequest):
    ...
    system_prompt = (
        ...
        "당신은 제공된 분석 결과를 바탕으로 고객사에 제출할 취약점 분석 리포트를 한국어로 작성해야 합니다. 반드시 다음 구조로, 오로지 HTML로만 작성해주세요.:\n"
        ...
        "출력은 <html> 태그로 시작해서 </html> 태그로 끝나야 합니다. 마크다운 문법인 ```html로 절대 시작하면 안 됩니다."
        "또한 필요한 경우 표를 적절히 사용해서 가독성을 높여주세요."
        "보고서의 시작 부분에 생성 날짜, URL, 타입 등이 들어있는 메타데이터 표를 추가해주세요."
    )
    user_content = (
        f"생성 날짜: {datetime.now().strftime('%Y-%m-%d')}\n"
